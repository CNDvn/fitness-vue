...
const payload = ref({ name: "", username: "", password: "", passwordAgain: "" })
...
const submitForm = async () => {
  await fetchAPI().post(`${baseUrl}/v1/api/auth/sign-up`, { ...payload.value })
  props.changeIsLogin()
}
</script>
    ...
    <div class="bg-white w-96 rounded-lg shadow-lg p-6">
      ...
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-xl font-bold text-gray-800">
          {{ i18n[languageStore.currentLanguage].signUpFitFuel }}
        </h1>
      ...
      <form @submit.prevent="submitForm">
        <div class="mb-4">
          <div class="flex items-center border rounded-lg px-3 py-2">
            <IconMe class="icon" />
            <input type="text" label="username" v-on:change="changeData"
        ...
        <div class="mb-4">
          <div class="flex items-center border rounded-lg px-3 py-2">
            <IconMe class="icon" />
            <input type="text" label="name" v-on:change="changeData"
              class="w-full ml-2 bg-transparent outline-none text-sm text-gray-800"
              :placeholder="i18n[languageStore.currentLanguage].name" />
          </div>
        </div>
        ...
        <div class="mb-4">
          <div class="flex items-center border rounded-lg px-3 py-2">
            ...
            <input type="password" label="password" v-on:change="changeData"
              class="w-full ml-2 bg-transparent outline-none text-sm text-gray-800"
        ...
        <div class="mb-4">
          <div class="flex items-center border rounded-lg px-3 py-2">
            <IconPassword class="icon" />
            <input type="password" label="passwordAgain" v-on:change="changeData"
              class="w-full ml-2 bg-transparent outline-none text-sm text-gray-800"
              :placeholder="i18n[languageStore.currentLanguage].passwordAgain" />
          </div>
        </div>

        <button type="submit" class="w-full bg-gray-400 text-white py-2 rounded-lg font-semibold cursor-not-allowed">
          {{ i18n[languageStore.currentLanguage].signUp }}
        </button>
